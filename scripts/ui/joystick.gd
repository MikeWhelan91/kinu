...
var run: NestRun
## For sticks outside a run (the Kinu Catcher): called with (deflection) every frame, and whether
## the stick takes a touch right now. Used instead of `run` when set.
var steer: Callable
var accepting: Callable
...
func _live() -> bool:
	if steer.is_valid():
		return not accepting.is_valid() or accepting.call()
	return is_instance_valid(run) and run.state == "aim"

func _physics_process(delta: float) -> void:
	if steer.is_valid():
		steer.call(deflection if _live() else Vector2.ZERO)
		return
