...
## UMP's authoritative gate, including consent retained from a previous session.
## Requires the CritterScale native consent patch in tools/patches.
func is_sdk_initialized() -> bool:
	return _plugin_singleton != null and _plugin_singleton.has_method("is_sdk_initialized") and _plugin_singleton.is_sdk_initialized()


func can_request_ads() -> bool:
	if _plugin_singleton == null or not _plugin_singleton.has_method("can_request_ads"):
		GmpLogger.log_error("AdMob native consent patch is missing; ads remain disabled")
		return false
