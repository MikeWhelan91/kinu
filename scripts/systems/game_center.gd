...
const LEADERBOARD_ID := "kinu.leaderboard"
...
# Posts a score to the Kinu Tumble leaderboard. Silently does nothing if the
...
func submit_score(value: int) -> void:
	...
	ClassDB.class_call_static("GKLeaderboard", "load_leaderboards", PackedStringArray([LEADERBOARD_ID]),
		func(leaderboards, error):
			if error or leaderboards.is_empty(): return
			leaderboards[0].call("submit_score", value, 0, local_player,
				func(submit_error):
					if submit_error: push_warning("Game Center score submit failed: %s" % submit_error))
	)

# Presents Apple's native leaderboard sheet. Fire-and-forget, matching the
# extension's own convention.
func show_leaderboard() -> void:
	if not available: return
	ClassDB.class_call_static("GKGameCenterViewController", "show_leaderboard_time_period", LEADERBOARD_ID, SCOPE_GLOBAL, TIME_SCOPE_ALL_TIME)
