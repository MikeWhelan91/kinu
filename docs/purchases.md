...
the same purchase screen. The back arrow returns to the originating page.
...
Balances and cosmetic ownership currently use the game's local save and backup.
Finished consumable purchases cannot reconstruct an unspent balance after the
...
Use `tests/visual.tscn -- screen=beans aspect=393x852` for the purchase layout.
...
be made as part of automated verification. All five IAPs were marked Prepare for
Submission in the owner's setup screenshot and require App Review before sales.
